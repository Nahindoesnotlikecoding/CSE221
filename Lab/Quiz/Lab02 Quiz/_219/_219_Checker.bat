...
for %%a in (py cpp java) do (
    powershell -Command "if (Select-String -Path '.\EnterIDandLanguage.txt' -Pattern '%%a') { exit 1 } else { exit 0 }" || (
        for %%b in (sort main scan print input output file read write thread process) do (
            powershell -Command "if (Select-String -Path '.\_219_Solution.%%a' -Pattern '%%b') { exit 1 } else { exit 0 }" || (
        ...
        if %%a==cpp (
            c++ _219_Solution.cpp _219.cpp && .\a.exe
            del /q /s /a /f .\*.exe >nul
        )
        if %%a==java (
            javac _219_Solution.java _219.java && java _219
